...
class Laser(pygame.sprite.DirtySprite):
    def __init__(self, filename, ship) -> None:
        ...
        self.speed_x = round(ship.speed_x - sin(radians(ship.rotation)), 0) * 2
        self.speed_y = round(ship.speed_y - cos(radians(ship.rotation)), 0) * 2
...
class Game(object):
    ...
    def draw(self):
        self.allsprites = pygame.sprite.LayeredDirty((self.space_ships_group, self.asteroid_small_group, self.asteroid_medium_group, self.asteroid_big_group, self.laser_group))
